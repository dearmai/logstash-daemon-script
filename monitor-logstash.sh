...
function init() { 
    ...
    if [ -f "${TARGET_PATH}/bin/logstash" ]; then
        return 0
    else
...
function check_netstat_beats() {
    local netstat=$(netstat -nat | grep ":${LS_BEATS_PORT} " | grep "LISTEN" | wc -l)
...
function check_netstat_api() {
    local netstat=$(netstat -nat | grep ":${LS_API_PORT} " | grep "LISTEN" | wc -l)
    if [ ${netstat} -eq 0 ]; then
        echo "die"
        return 1
    fi
...
function show_status() {
    echo -e "$(date +%Y-%m-%dT%H:%M:%S%z)\tPID : $(get_pid)\tNet Status : beats - $(check_netstat_beats), api - $(check_netstat_api)"
}
...
case $1 in
    status)
        ...
        echo "Network port status(${LS_BEATS_PORT}) : $(check_netstat_beats)"
        ;;
    run)
        print "Run"
        cd ${TARGET_PATH}
    ...
    start)
        ...
        cd ${TARGET_PATH}
        eval nohup "bin/logstash" > /dev/null 2>&1 "&"
        echo $! > ${PID_PATH}
    ...
    stop)
        ...
        if [ $? -eq 0 ]; then
            ...
            do
                status_pidfile > /dev/null 2>&1
                show_status
                if [ $(check_netstat_beats) = "die" ] && ! [ -e ${PID_PATH} ]; then
                    print "${TARGET_NAME} 가 정상적으로 종료 됨"
                    break;
                fi
                sleep 1
            done;
